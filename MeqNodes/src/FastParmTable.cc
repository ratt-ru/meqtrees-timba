...
#ifndef HAVE_PARMDB
#include <MeqNodes/FastParmTable.h>
...
{
  Throw(Debug::ssprintf(message.c_str(),table_name_.c_str())+string(": ")+gdbm_strerror(gdbm_errno));
}
...
int FastParmTable::getFunklets (vector<Funklet::Ref> &funklets,const string& parmName,const Domain& domain)
{
  ...
  char key[sizeof(int)+parmName.length()+1];
  int *key_dom = reinterpret_cast<int*>(key);
  parmName.copy(key+sizeof(int),parmName.length());
  key[sizeof(key)-1] = 0;
  datum db_key = { key,sizeof(key) }; 
    ...
    if( domain_match_[i] )
    {
      *key_dom = i;
      ...
      try
      {
        ...
        funklets[ifunk].copy(DynamicTypeManager::construct(0,bset));
        if( Debug(3) )
        {
                                       ...
                                       bool)
{
  ...
  *pdata = bset.size();
  ...
  // build up funklet database key
  char key[sizeof(int)+parmName.length()+1];
  *(reinterpret_cast<int*>(key)) = dom_id;
  parmName.copy(key+sizeof(int),parmName.length());
  key[sizeof(key)-1] = 0;
  datum db_key = { key,sizeof(key) }; 
...
bool FastParmTable::DomainEntry::match (const Domain &dom)
{
  for( int i=0; i<Axis::MaxAxis; i++ )
  {
...
bool FastParmTable::DomainEntry::overlaps (const Domain &dom)
{
  for( int i=0; i<Axis::MaxAxis; i++ )
  {
    if( defined[i] && dom.isDefined(i) )
