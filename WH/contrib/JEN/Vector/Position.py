...
class Position (Parameterization):
  ...
  def __init__(self, ns, name,
    ...
    if not y==None:
      self.dims[0] = 1
      self._add_parm('xpos', x, tags=['position'], solvable=solvable)
    if not y==None:
      self.dims[1] = 1
      self._add_parm('ypos', y, tags=['position'], solvable=solvable)
    if not z==None:
      self.dims[2] = 1
      self._add_parm('zpos', z, tags=['position'], solvable=solvable)
    if not t==None:
      self.dims[3] = 1
      self._add_parm('tpos', t, tags=['position'], solvable=solvable)
