...
-   ...
The key defines the (sub)menu structure, e.g. 'compile.aa.bb.cc'
...
class OptionManager (object):
    ...
    def __getitem__ (self, key):
        ...
        if False:
            ...
            # of the option.value (if it exists).
            ...
            if self.option.has_key(key):
                setattr(self, ukey, self.option[key].value)
    ...
    def TDLMenu (self, key=None, severe=False, trace=False):
    ...
    def hide(self, key=None, hide=True):
        ...
        if isinstance(key,bool):
            hide = key
            key = None
        menu = self.TDLMenu(key)
        if menu: return menu.hide(hide)
        option = self.TDLOption(key)
    ...
    def findkey (self, substring, keys=[], one=True, trace=False):
        ...
        """
        if trace:
            print '** .findkey(',substring,len(keys),'): ',
        found = []
        ...
        if not one:
            ...
            s = 'found more than one: '+str(found)
            raise ValueError, s
    ...
    def keys_obsolete (self, name=None, trace=False):
