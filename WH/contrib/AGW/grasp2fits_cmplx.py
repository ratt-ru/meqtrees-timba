...
# get beam number to test if we need to flip axes
        str_beam = filename[12:14]
        flip = False
        if int(str_beam) > 15:
          flip = True
...
#	print 'input grd file contains %d comments' % i
	...
	return data, (scale*180.0/math.pi), flip

def totalpower( data, flip, nx, mV=0, mH=0, col_no=0 ):
...
def main( argv ):
        data, scale, flip = getdata(argv[1])
	...
	if(nx*nx != size):
        ...
        try:
          ...
          col_no = 0
	Z, x_max, y_max = totalpower(data, flip, nx, mV, mH,col_no);
