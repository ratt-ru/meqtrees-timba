...
def forest_baseline_predict_trees(ns, interferometer_list, sources):
    ...
    for (ant1, ant2) in interferometer_list:
        ...
        for source in sources:
            ns.corrupted_vis_all(ant1,ant2,source.name) << \
                    Meq.MatrixMultiply(ns.E(ant1,source.name),
                                 ns.clean_visibility(ant1,ant2, source.name),
                                 ns.ctE(ant2, source.name))
            ns.corrupted_vis_mean(ant1,ant2,source.name) << \
                    Meq.MatrixMultiply(ns.E_mean(ant1,source.name),
                                 ns.clean_visibility(ant1,ant2, source.name),
                                 ns.ctE_mean(ant2, source.name))
            ns.corrupted_vis(ant1,ant2,source.name) <<Meq.Subtract(ns.corrupted_vis_all(ant1,ant2,source.name), ns.corrupted_vis_mean(ant1,ant2,source.name))
...
# I believe the following should be correct here
                ...
                ea_mean = ns.EA_mean(station, source_name, elem) << Meq.Sqrt(ns.mean_exp_v_gain(station,source_name)) 
                pass
            ns.E(station, source_name, elem) << Meq.Polar(ea,ep);
            ns.E_mean(station, source_name, elem) << Meq.Polar(ea_mean,ep);
#           if i == j:
#             create_refparms(ea);
#             create_refparms(ep);
            pass # for j ...
      ...
      ns.E_mean(station,source_name) << Meq.Matrix22(ns.E_mean(station, source_name, '11'),
                                             ns.E(station, source_name, '12'),
                                             ns.E(station, source_name, '21'),
                                             ns.E_mean(station, source_name, '22'))
      ns.ctE_mean(station, source_name) << Meq.ConjTranspose(ns.E_mean(station,source_name))
...
# HPBW of 3 arcmin = 0.00087266 radians 1145.9156 = 1 / 0.00087266
    ...
    ns.feed_offset << Meq.Constant(0.00261798)  # offset of 9 armin in Azimuth
#   ns.feed_offset << Meq.Constant(0.00174532)  # offset of 9 armin in Azimuth
#   ns.feed_offset << Meq.Constant(0.000)  # offset of 9 armin in Azimuth
...
def create_station_subtrees(ns,st):
  ...
  # then compute azimuth of offset feed as seen from this station 
  ns.AzEl_az_offset(st) << Meq.Add(ns.AzEl_az0(st), ns.feed_offset )

  # now create azimuth-evlevation node for the off-axis feed
  ns.AzEl_offset(st) << Meq.Composer(ns.AzEl_az_offset(st), ns.AzEl_el0(st))
...
# of nodes used to compute Az, El of field centre
  ...
  ns.mean_exp_v_gain(st,src.name) << Meq.Mean(ns.exp_v_gain(st,src.name))
