...
Utils.include_ms_options(tile_sizes=[1,30,48,60,96,192,480,960]);
...
# along with initial guesses
  beam_width = create_polc_ft(degree_f=0, degree_t=0, c00=0.012)
  beam_offset_l = create_polc_ft(degree_f=0, degree_t=0, c00=0.0005)
  beam_offset_m = create_polc_ft(degree_f=0, degree_t=0, c00=-0.0005)
  ns.width << Meq.Parm(beam_width, node_groups='Parm')
...
# ns.beam_angle = Meq.Constant(1.0)
  ...
  ns.width_sq << Meq.Sqr(1.0 / ns.width)
...
# voltage_gain = sqrt(exp(log16 * (L_gain + M_gain)))
  for src in SOURCES:
    ...
    ns.l_sq(src) << Meq.Sqr(l_prime)
    ns.m_sq(src) << Meq.Sqr(m_prime)

    # then multiply by width squared, for L, M
    ns.l_vpsq(src) << Meq.Multiply(ns.l_sq(src), ns.width_sq)
    ns.m_vpsq(src) << Meq.Multiply(ns.m_sq(src), ns.width_sq)
...
def _define_forest(ns):  
  ...
  # create array stuff
  array = Meow.IfrArray(ns,range(1,NUM_ANTENNAS+1))
...
# its complex conjugate for each antenna  / source combination
  ...
  ns.inspect_residuals << Meq.Composer(
    dims=[0],
    plot_label=["%s-%s"%(p,q) for p,q in array.ifrs()],
    *[ns.residual(p,q) for p,q in array.ifrs()]
  );
  ns.inspect_predicts << Meq.Composer(
    dims=[0],
    plot_label=["%s-%s"%(p,q) for p,q in array.ifrs()],
    *[ns.predict_ok(p,q) for p,q in array.ifrs()]
  );
  ...
  ns.VisDataMux = Meq.VisDataMux(
    pre=Meq.ReqMux(ns.inspect_spigots,ns.inspect_predicts),
    post=ns.inspect_residuals,
  ...
  pg = Bookmarks.Page("predicts Inspectors",1,2);
  pg.add(ns.inspect_predicts,viewer="Collections Plotter");
...
def _test_forest(mqs,parent):
  ...
  solvables =  [ "width", "offset_l", "offset_m" ];
