...
# get complex weights for beams
  ...
  for p in ANTENNAS:
    ...
    ns.Gx_n(p) << Meq.Add(*[ns.Gxw(p,k) for k in BEAMS]) / (ns.sum_wt_x * I_parm_max_x)
    ns.Gy_n(p) << Meq.Add(*[ns.Gyw(p,k) for k in BEAMS]) / (ns.sum_wt_y * I_parm_max_y)
  ...
  for p,q in IFRS:
    ...
    ns.predict(p,q) << Meq.MatrixMultiply(ns.G_n(p),ns.Gt_n(q)) 
...
#     ms_name          = 'TEST_VLA_60_480.MS',
#     ms_name          = 'TEST_VLA_0_0.5_60_480.MS',
