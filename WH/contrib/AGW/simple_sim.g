...
#    xx := [411.100006,134.110001,268.309998,439.410004,644.210022,880.309998,
...
#  26.3299999]  
  ...
  -60.48999977,-120.5299997,10.25999999, 40.23000002,90.5,160.6800003, 260.3299999]  
    ...
    mysim.settimes(integrationtime='60s', usehourangle=T, referencetime=ref_time);
    ...
    while(starttime<14400) {
      mysim.observe('test_image', 'LBAND',
...
mkimage:=function(msname,RA=RA0,Dec=DEC0)
{
    include 'imager.g';
    ...
    imsize := 1024
    dir0    := dm.direction('j2000',  RA, Dec);
    myimager.setimage(cellx=cellsize, celly=cellsize, nx=imsize, ny=imsize, phasecenter=dir0)
    restored_image := 'test_im.restored'
    model_image := 'test_im.clean'
    residual_image := 'test_im.residual'
    myimager.clean(image=restored_image, model=model_image,
                  residual=residual_image)

    include 'image.g'
    myimage := image(restored_image)
    myimage.tofits('test_im.fits')
    myimage.view()
}
...
shell('rm -rf test_im* TEST_CLAR.MS mymodel.cl')
...
print '*** calling simms ***'
simms('TEST_CLAR.MS','mymodel.cl',noise='0.01Jy');
