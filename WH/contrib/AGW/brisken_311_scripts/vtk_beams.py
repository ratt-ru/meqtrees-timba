...
#=============================
  ...
  def array_plot(self, caption, incoming_array, scale):
...
# first display
  ...
  def setAxisIncrements(self, axis_increments):
    ...
    """
### ***
# remember - the SetSpacing method does not exist
# in standard VTK
### ***
#   if not axis_increments[0] is None:
#     self.axes.GetXAxisActor2D().SetSpacing(axis_increments[0])
#     self.spacing_x = axis_increments[0]
#   if not axis_increments[1] is None:
#     self.axes.GetYAxisActor2D().SetSpacing(axis_increments[1])
...
def main( argv ):
  if has_vtk:
	...
	Z = totalpower(data, nx);
        ...
        display.show()
        display.AddUpdate()
        display.array_plot('test of brisken',Z, scale)
        app.exec_loop()
  else:
    print ' '
