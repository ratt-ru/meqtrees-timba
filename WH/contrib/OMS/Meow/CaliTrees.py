...
def define_solve_correct_tree (ns,predict,
  ...
  spigot = array.spigots(flag_bit=1);
  ...
  sink = array.sinks(children=output_nodes,flag_bit=1,output_col='DATA');
  
  vdm = ns.VisDataMux << Meq.VisDataMux(post=inspector,*[sink(p,q) for p,q in array.ifrs()]);
