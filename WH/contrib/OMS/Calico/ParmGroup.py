...
class ParmGroup (object):
  def __init__ (self,label,nodes=[],name=None,
                     ...
                     table_name="calib.mep",table_in_ms=True,
                     **kw):
    self.label = label;
    ...
    if individual:  
      ...
      for parm in nodes:
        ...
        opts.tdloption_namespace = label+"."+parm.name;
        self._option_list.append(
          TDLMenu("Solve for %s"%parm.name,
            ...
            TDLOption('time_deg','Polynomial degree, time',0,more=int,namespace=opts),
            TDLOption('freq_deg','Polynomial degree, freq',0,more=int,namespace=opts),
  ...
  def make_cmdlist (self):
    ...
    if self._individual:
      ...
      nonsolve = [ parm for parm in self.nodes if not self._parm_opts[parm.name].solvable ];
      if nonsolve:
  ...
  def run_solution (self,mqs,mssel=None,vdm=None,tiling=None,wait=False):
    ...
    self.solver_control.update_state(mqs,cmdlist=self.make_cmdlist());
    # run the VisDataMux
