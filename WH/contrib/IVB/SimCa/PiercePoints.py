...
class PiercePoints(MIM_model):
    ...
    def make_pp(self,ref_station=None):
		...
		if  not diff.initialized():
                ...
                # correct. draw triangle from piercepoint to center earth, and along the line 
                # connecting pp and antenna with 90 degree angle between center earth and that line.
                ...
                # correct. draw triangle from center earth to pierce point and along vector a 
                # (=xyz_station), with a 90 degree angle between that line and piercepoint
                
                if ref_station:
                    ns['pp'](src,station) << Meq.MatrixMultiply(ref_rot_matrix,xyz(station) + diff_vector*scale);
                else:
                    ns['pp'](src,station) << Meq.Add(xyz(station),diff_vector*scale, cache_policy=100);
