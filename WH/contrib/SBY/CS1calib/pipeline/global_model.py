...
def point_and_extended_sources_solvable(ns,lsm,tablename=''):
  ...
  for pu in plist:
     ...
     if f0==0:
       f0=150e6
     if eX!=0 or eY!=0 or eP!=0:
         source_model.append( GaussianSource(ns,name=pu.name,I=Meow.Parm(sI), Q=sQ, U=sU, V=sV,
                  Iorder=0, direction=Direction(ns,pu.name,ra,dec,parm_options=parm_options),
                  spi=SIn,freq0=f0,
                  size=[eX,eY],phi=eP,
                  parm_options=parm_options));
       ...
       # point sources
       if RM==0: 
         source_model.append( PointSource(ns,name=pu.name,I=Meow.Parm(sI), Q=sQ, U=sU, V=sV,
                  direction=Direction(ns,pu.name,ra,dec),
                  spi=SIn,freq0=f0));
