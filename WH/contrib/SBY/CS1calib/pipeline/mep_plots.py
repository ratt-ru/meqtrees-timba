...
def create_residual_plots(debug_file=None,sig_zeromean=True,sig_rescale=True,sig_logplot=True,sig_percent=0.7,npages=4):
      ...
      #ff.close()
      # parmtable
      (signals0,names)=read_parmtable(debug_file)
      # calculate dummy t
      nx=len(signals0[0])
      #print nx
      t=arange(0.0,nx,1)
...
def read_parmtable(infile, dtype='float32'):
  import pycasatable
  from itertools import chain
  import numpy

  tt=pycasatable.table(infile)
  ...
  print names
  for i in xrange(ncols):
   data[i]=numpy.cast['float32'](data[i])
...
if __name__ == '__main__':
  ...
  argc=len(sys.argv)
  if argc>1:
   create_residual_plots(sys.argv[1],npages=10,sig_logplot=False)
