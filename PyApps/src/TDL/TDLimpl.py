...
class _NodeDef (object):
  """this represents a node defintion, as returned by a node class call""";
...
class _NodeStub (object):
  ...
  def __init__ (self,fqname,basename,scope,*quals,**kwquals):
    ...
    self._debuginfo = "%s(%s)" % (basename,_identifyCaller());
  ...
  def __lshift__ (self,arg):
    ...
    if arg is None:
      raise NodeDefError,self._debuginfo+": cannot bind node to something of type "+type(arg).__name__;
    (children,initrec) = (arg.children,arg.initrec);
    if not self.initialized():
      try: self.classname = getattr(initrec,'class');
      except AttributeError: 
        raise NodeDefError,self._debuginfo+": init record missing class field";
