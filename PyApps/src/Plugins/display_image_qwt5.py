...
#% $Id: display_image.py 6482 2008-11-01 15:40:00Z twillis $ 
...
from ComplexScaleDraw_qwt5 import *
from QwtPlotCurveSizes_qwt5 import *
from QwtPlotImage_qwt5 import *
...
#       self.setLegendPos(Qwt.Right)
        self.setAxisTitle(Qwt.QwtPlot.xBottom, 'Array/Channel Number')
        self.setAxisTitle(Qwt.QwtPlot.yLeft, 'Array/Sequence Number')
      ...
      if len(self.chis_plot) > 0:
        ...
        if not zoom:
          ...
          if self.log_axis_chi_0:
            self._menu.setItemChecked(menuid, True)
            self.setAxisOptions(Qwt.QwtPlot.yRight, QwtAutoScale.Logarithmic)
          else:
            self._menu.setItemChecked(menuid, False)
            self.setAxisOptions(Qwt.QwtPlot.yRight, QwtAutoScale.None)

          menuid = self.menu_table['Toggle log axis for solution vector']
          if self.log_axis_solution_vector:
            self._menu.setItemChecked(menuid, True)
            self.setAxisOptions(Qwt.QwtPlot.xTop, QwtAutoScale.Logarithmic)
          else:
            self._menu.setItemChecked(menuid, False)
            self.setAxisOptions(Qwt.QwtPlot.xTop, QwtAutoScale.None)
...
# create x_index defaults for array plots 
        ...
        self.setAxisTitle(Qwt.QwtPlot.yRight, 'x cross-section value')
        if self.real_xsection_selected:
          self.setCurveYAxis(self.xrCrossSection, Qwt.QwtPlot.yRight)
        if self.complex_type:
          if self.xiCrossSection is None and self.imag_xsection_selected:
            ...
            plot_curve.setSymbol(QwtSymbol(QwtSymbol.Ellipse, 
               QBrush(Qt.black), QPen(Qt.black), QSize(q_symbol_size,q_symbol_size)))
            self.setCurveYAxis(self.xiCrossSection, Qwt.QwtPlot.yRight)
        ...
        self.enableAxis(Qwt.QwtPlot.xTop)
        self.setAxisTitle(Qwt.QwtPlot.xTop, 'y cross-section value')
        self.setCurveYAxis(self.yCrossSection, Qwt.QwtPlot.yLeft)
        self.setCurveXAxis(self.yCrossSection, Qwt.QwtPlot.xTop)
        if self._vells_plot:
...
# add solver metrics info?
          ...
          if self.array_flip:
            ...
            self.setCurveXAxis(key, Qwt.QwtPlot.xBottom)
            self.setCurveData(key, plot_data, self.iteration_number)
          else:
            self.setCurveYAxis(key, Qwt.QwtPlot.xBottom)
            self.setCurveXAxis(key, Qwt.QwtPlot.yLeft)
...
#         print 'removed chis_plot curve with key ', self.chis_plot[i]
      ...
      self.setAxisTitle(Qwt.QwtPlot.yRight, 'chi_0')
      self.setAxisTitle(Qwt.QwtPlot.xTop, 'amplitude of solution vector')
      ...
      for i in range(shape[1]):
        ...
        if self.array_flip:
          ...
          self.setCurveXAxis(key, Qwt.QwtPlot.xTop)
          curve.setData(plot_data,chi_data)
        else:
          self.setCurveYAxis(key, Qwt.QwtPlot.xTop)
          self.setCurveXAxis(key, Qwt.QwtPlot.yRight)
...
# put in a line where cross sections are selected
      if not self.x_arrayloc is None:
          ...
          self.x_sec_marker.setLineStyle(Qwt.QwtPlotMarker.HLine)
          self.x_sect_marker.setValue(self.x_arrayloc,0.0)
      ...
      if not self.y_arrayloc is None:
          ...
          self.y_sect_marker.setValue(self.y_sect_marker, self.y_arrayloc)
...
# text marker giving mean and std deviation of array
        ...
        ylb = self.axisScaleDiv(Qwt.QwtPlot.yLeft).hBound()
        xlb = self.axisScaleDiv(Qwt.QwtPlot.xBottom).hBound()
        m.setValue(ylb, xlb)
...
# no legends by default
    ...
    def cleanup(self):
      self.clear()        # removes all curves and markers
      self.xrCrossSection = None
      self.xrCrossSection_flag = None
      self.xiCrossSection = None
      self.yCrossSection = None
...
# first test for real or complex
      ...
      if self.complex_type:
        toggle_id = self.menu_table['Toggle axis rotate']
        self._menu.setItemVisible(toggle_id, False)
      print 'self.complex_type = ', self.complex_type
...
# test if we have a 2-D array
      ...
      if self.is_vector == False:
        ...
        print 'self.complex_divider ', self.complex_divider
...
#           if self.axes_rotate:
#             self.setAxisOptions(Qwt.QwtPlot.xBottom, QwtAutoScale.Inverted)
            self.setAxisTitle(Qwt.QwtPlot.xBottom, self._x_title)
...
# call reset_zoom ...
        ...
        self.setAxisAutoScale(Qwt.QwtPlot.yLeft)
        self.setAxisAutoScale(Qwt.QwtPlot.yRight)
#       self.setAxisScaleDraw(Qwt.QwtPlot.xBottom, QwtScaleDraw())
#       self.setAxisScaleDraw(Qwt.QwtPlot.yLeft, QwtScaleDraw())
        self._x_auto_scale = True
        self._y_auto_scale = True
...
# we have a complex vector
        if self.complex_type:
          ...
          self.enableAxis(Qwt.QwtPlot.xBottom)
          if self.ampl_phase:
            self.setAxisTitle(Qwt.QwtPlot.yLeft, 'Value: Amplitude (black line / red dots)')
            self.setAxisTitle(Qwt.QwtPlot.yRight, 'Value: Phase (blue line / green dots)')
          ...
          self.xrCrossSection.setAxis(QwtPlot.xBottom, QwtPlot.yLeft)
          self.yCrossSection.setAxis(QwtPlot.xBottom, QwtPlot.yRight)
          self.xrCrossSection.setSymbol(Qwt.QwtSymbol(Qwt.QwtSymbol.Ellipse, QBrush(Qt.red),
                     QPen(Qt.red), QSize(q_symbol_size,q_symbol_size)))
          self.yCrossSection.setSymbol(Qwt.QwtSymbol(Qwt.QwtSymbol.Ellipse, QBrush(Qt.green),
                     QPen(Qt.green), QSize(q_symbol_size,q_symbol_size)))
          ...
          if not self._flags_array is None:
            ...
            self.xrCrossSection_flag.setAxis(QwtPlot.xBottom, QwtPlot.yLeft)
            self.yCrossSection_flag.setAxis(QwtPlot.xBottom, QwtPlot.yRight)
            self.xrCrossSection_flag.setSymbol(Qwt.QwtSymbol(Qwt.QwtSymbol.Ellipse, QBrush(Qt.red),
                     QPen(Qt.red), QSize(q_symbol_size,q_symbol_size)))
            self.yCrossSection_flag.setSymbol(Qwt.QwtSymbol(Qwt.QwtSymbol.Ellipse, QBrush(Qt.green),
                     QPen(Qt.green), QSize(q_symbol_size,q_symbol_size)))
          if self.ampl_phase:
          ...
          if not self._flags_array is None:
            ...
            print 'assigning data to complex plot vectors'
...
# stuff for flags
          if not self._flags_array is None:
            ...
            self.setCurveYAxis(self.real_flag_vector, Qwt.QwtPlot.yLeft)
            ...
            self.setCurveYAxis(self.imag_flag_vector, Qwt.QwtPlot.yRight)
...
# stuff for flags
            ...
            self.setCurveStyle(self.real_flag_vector, QwtCurve.Dots)
            self.real_flag_vector.setAxis(QwtPlot.xBottom, Qwt.QwtPlot.yLeft)
            self.real_flag_vector.setSymbol(Qwt.QwtSymbol(Qwt.QwtSymbol.XCross, QBrush(Qt.black),
                     QPen(Qt.black), QSize(q_flag_size, q_flag_size)))
