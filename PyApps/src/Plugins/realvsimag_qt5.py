...
from qwt.qt.QtGui import QApplication, QLabel, QSizePolicy, QBrush, QFrame
from qwt.qt.QtGui import QPen, QWidget, QFont, QFontInfo, QMenu, QActionGroup, QAction, QMessageBox
from qwt.qt.QtCore import Qt, QTimer, QSize, QObject, pyqtSignal
...
# specified string values and internal enumerated values
  ...
  show_results_selector = pyqtSignal()
  color_table = {
        'none': None,
        'black': Qt.black,
        'blue': Qt.blue,
...
# function calls
  ...
  def __initTracking(self):
        ...
        self.spy.MouseMove.connect(self.onMouseMoved)
...
# we are zooming, so remove any markers
  ...
  def __initZooming(self):
        ...
        self.picker.selected[QPointArray].connect(self.selected)
...
#                QwtEventPattern.MousePattern(Qt.MidButton, Qt.ShiftButton),
  ...
  def __initContextMenu(self, Test=False):
        ...
        self._modify_plot_parameters = QAction('Modify Plot Parameters',self.plot)
        self._menu.addAction(self._modify_plot_parameters)
        self._modify_plot_parameters.setData(str(menu_id))
        self._modify_plot_parameters.triggered.connect(self.update_display)


        menu_id = self.menu_table['Reset zoomer']
        self._reset_zoomer = QAction('Reset zoomer',self.plot)
        self._menu.addAction(self._reset_zoomer)
        self._reset_zoomer.setData(str(menu_id))
        self._reset_zoomer.setVisible(False)
        self._reset_zoomer.triggered.connect(self.update_display)

        menu_id = self.menu_table['Undo Last Zoom']
        self._undo_last_zoom = QAction('Undo Last Zoom',self.plot)
        self._menu.addAction(self._undo_last_zoom)
        self._undo_last_zoom.setData(str(menu_id))
        self._undo_last_zoom.setVisible(False)
        self._undo_last_zoom.triggered.connect(self.update_display)

        menu_id = self.menu_table['Toggle Legend']
        self._toggle_legend = QAction('Toggle Legend',self.plot)
        self._menu.addAction(self._toggle_legend)
        self._toggle_legend.setData(str(menu_id))
        self._toggle_legend.triggered.connect(self.update_display)

        menu_id = self.menu_table['Save Display in PNG Format']
        self._save_display = QAction('Save Display in PNG Format',self.plot)
        self._menu.addAction(self._save_display)
        self._save_display.setData(str(menu_id))
        self._save_display.triggered.connect(self.update_display)
        
#       printer = QAction('Print plot',self.plot);
#       printer.triggered.connect(self.printPlot)
...
##                     self.zoom)
  ...
  def update_display(self):
    ...
    action = QObject.sender(self.plot)
    menuid = int(action.data())
    ...
    if menuid == self.menu_table['Save Display in PNG Format']:
      self.plot.save_display.emit(self._plot_title)
      return True
...
# start or stop blinking
  ...
  def toggleResults(self):
    if self.setResults:
      ...
      self.setResults = True
    self.plot.show_results_selector.emit(self.setResults)
...
#       self.plot.setOutlineStyle(Cross)
          ...
          if 'plot' in self._attrib_parms:
            ...
            if self._plot_color is None and 'color' in self._plot_parms:
              ...
              if self._plot_color in self.color_table:
                ...
                mb_reporter = QMessageBox.warning(None, "RealVsImag", Message)
...
#         plot_curve.setSymbol(QwtSymbol(plot_symbol, QBrush(self._plot_color),
          curve.setSymbol(QwtSymbol(plot_symbol, QBrush(self._plot_color),
...
# get the equivalent for the imaginary data
         ...
         if (plot_flag_r_keys[i] in self.flag_plot_dict) == False:
           curve = QwtPlotCurve(plot_flag_r_keys[i])
           self.flag_plot_dict[plot_flag_r_keys[i]] = curve
           curve.attach(self.plot)
           curve.setPen(QPen(Qt.black))
           curve.setStyle(QwtPlotCurve.Dots)
           curve.setSymbol(QwtSymbol(QwtSymbol.XCross, QBrush(Qt.black),
...
# a test routine
  def go_errors(self, counter):
      ...
      curve = ErrorBar_qt4.ErrorBarPlotCurve(
          ...
          curveSymbol = QwtSymbol(QwtSymbol.Cross,
                                    QBrush(),
                                    QPen(Qt.yellow, 2),
                                    QSize(7, 7)),
