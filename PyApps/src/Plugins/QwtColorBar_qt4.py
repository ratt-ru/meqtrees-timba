...
#         self.setAxisTitleFont(axis,font);
	...
	# set axis parameters
        ...
        self.setAxisLabelAlignment(Qwt.QwtPlot.yLeft, Qt.Qt.AlignTop)
        # default color bar
...
#       self.setMaximumWidth(self.sizeHint().width() * 1.5)
        ...
        if self._mainwin:
          ...
          self._menu = Qt.QMenu(None);

        self._unzoom_action = Qt.QAction('unzoom intensity range', self)
        self._menu.addAction(self._unzoom_action)
        self._unzoom_action.setVisible(False)
        self.connect(self._unzoom_action, Qt.SIGNAL('triggered()'), self.unzoom)

        self._lock_colorbar = Qt.QAction('lock colorbar scale', self)
        self._menu.addAction(self._lock_colorbar)
        self.connect(self._lock_colorbar, Qt.SIGNAL('triggered()'), self.lock_colorbar)
#       self._menu.insertItem(pixmaps.close_lock.iconset(),'lock colorbar scale ', toggle_id)

        self._unlock_colorbar = Qt.QAction('unlock colorbar scale', self)
...
#       self._menu.insertItem(pixmaps.open_lock.iconset(),'unlock colorbar scale ', toggle_id)
        self.connect(self._unlock_colorbar, Qt.SIGNAL('triggered()'), self.unlock_colorbar)
...
#       if abs(max - min) < 0.00005:
    ...
    def handleRangeParms(self, parms):
      limits = parms[0]
      colorbar_number = parms[1]
      log_scale = parms[2]
      ampl_phase = parms[3]
