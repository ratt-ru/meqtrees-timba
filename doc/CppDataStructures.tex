...
\subsection{Naming conventions}
  ...
  \item In C++, standard data structures \& nodes reside in \qq{namespace Meq}.
    ...
    uses a {\tt Meq} prefix. Thus, the \qq{Meq::\Request} class in C++ is
    registered as a ``\qq{MeqRequest}'' in the DMI type system, and  has a 
    \qq{meq.request()} counterpart in Glish.
...
\subsection{Glish \qq{meq} objects: meqtypes.g}
  ...
  classes rather than generic \qq{DataRecord}s. Specifically, the
  \qq{::dmi\_actual\_type} attribute is set to a string which gives the DMI
  object type. 

  Thus, it is possible to construct a record in Glish, tag it with
  \qq{::dmi\_actual\_type}, pass it to C++, and have it auto-magically
  ...
  attributes (data members) in C++. The file \qq{meq/meqtypes.g} defines  some
  Glish ``constructor'' functions which create properly formed records:

  \begin{description}
  
  \item[\qq{meq.domain()}] creates a Domain object. This is actually a vector
...
\section{The details of Node::execute()}
  ...
  \item If node has children, calls the virtual \qq{pollChildren()} method to
    ...
    with the exception of specialized ``control'' nodes such as \qq{Sink} and
    \qq{Solver}.
...
\subsubsection{MeqParm state}
  ...
  \item[\qq{auto_save}] (bool, optional) If true, any updates to the parm's 
...
\subsubsection{Selecting polcs}
  ...
  \item If no MEP table and no default polc has been set in the state record,
    ...
    \qq{polcs} field of the state record), or fail if the list is empty. 
  ...
  \item Otherwise, the parm will first check if the current polc list contains
    ...
    \item If the \qq{inf_domain} flag is set, the polc has an infinite domain, 
      and will be re-used directly.
      
    \item If the \qq{grow_domain} flag is set, the polc allows for ``growing
