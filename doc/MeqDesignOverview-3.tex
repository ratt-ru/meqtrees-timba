...
\subsection{Auto-resampling modes}
  ...
  (see \ref{sec:children}). The specified child becomes the resolution
  driver. If the child returns a resolution-free result (see
  \ref{sec:result-empty-cells}), then a fail is reported.
  ...
  A few utility nodes may be used to change resolution mid-tree.
  ...
  proxy. 
   ...
   Data access nodes (\qq{Sink} and \qq{Spigot}) are coupled. For each
  ...
  able to return a \Result\ with the same cells.
   ...
   For the time being, we'll only support integral resolutions -- i.e.,
