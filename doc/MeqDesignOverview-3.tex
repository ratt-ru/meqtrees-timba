\chapter{Executing Requests}
  ...
  MeqTree kernel. A solid understanding of how it works is necessary for
  both tree design and node development. An understanding is highly
  recommended to advanced users that need to deal directly with trees.}
...
{\addtocounter{execstep}{1}\highlightbox{\mbox{\bf Step \arabic{execstep}. }
#1}}
  
\subsection{Checking the cache}
  
  \ExecStep{Compare the request id to the id of the previous request, if any. Sets a
  local ``new request'' flag that affects further logic below.}
...
\subsection{Polling children}
  ...
  \item Calls \qq{execute()} (with \qq{req}) on all the child nodes,
  collects their \Result{}s into the \qq{child\_results} vector, and
  ...
  \qq{execute()} returns immediately with the same return value. Otherwise,
  it proceeds to the next step:

  \ExecStep{If auto-resampling is enabled (see \ref{sec:resample}), compare
...
\subsection{Evaluating Cells}
  ...
  \ExecStep{If the request contains a \qq{cells} command (containing a
  \Cells\ object), call the virtual \qq{getResult()} handler to process
  the command, passing in the vector of child \Result{}s
  returned by \qq{pollChildren()}. Bitwise-ORs the return value with 
  the node's current depmask, and the cumulative result code from the 
  children (if one was returned at the previous step).}

\subsection{Handling exceptions}

  \ExecStep{If an exception is thrown at any stage of the process,
    \qq{execute()} will catch it, create an output \Result\ with a
    fail-result describing the exception, and return \RES{FAIL}. Thus,
    throwing an exception is a normal way for \qq{setState()} or
    \qq{processCommands()} or \qq{getResult()} to indicate failure.}

\subsection{Setting cache}

  \ExecStep{Upon exit, optionally stores the output \Result\ and result code
    in the cache (see caching issues, below.)}
