...
\section{Caching issues}
  ...
  Caching policy is settable on a per-node basis, via the \qq{cache\_policy}
...
\subsection{Smart caching behaviour}
  ...
  reasonable assumption: a \qq{Sink} always knows that its next request is
  going to have a different domain/cells, a \qq{Solver} knows that its next
...
\subsection{Known and active symdeps} 
  ...
  class's constructor, by calling the \qq{setKnownSymDeps()} method. 
...
\subsection{Generating symdep masks}
  ...
  \item Once all the nodes and trees have been created, a request containing
    ...
    required to resolve named children). This is done by the \qq{meqserver}
