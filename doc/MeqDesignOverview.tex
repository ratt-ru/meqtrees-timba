...
\section{Basic concepts}
  ...
  classes) can be defined in C++ terms only.

  Congruity allows {\bf transparency}: most of the inner workings of a tree are
  readily accessible from the scripting side. This allows for very elaborate
  monitoring schemes, and is a great debugging aid when something goes wrong.
...
\subsection{Data types}
  ...
  languages. We'll define ``legal data objects'' as:
...
\subsection{HIIDs}
  ...
  itself in string form, while for IDs $<$0, a global {\em vocabulary} (i.e. map
  from strings to numbers) is maintained in the development tree. Another way
  to look at it is that negative IDs represent {\em atomic concepts}, or words.
  Thus, any HIID can be viewed as a mix of words (from a fixed though rather
  large vocabulary!) and numbers.
  ...
  const declarations for atomic IDs. For example,
...
\subsection{Naming conventions}
  ...
  \item Even though the languages we use are case-sensitive, HIIDs aren't.
    Another reason to not rely on character case to distinguish identifiers is
    ...
    \qq{meq.request} in Glish. Thus we should always avoid situations where
    different entities have names that differ only in character case.
...
\subsection{1-based and 0-based indexing}
  ...
  {\bf The Glish/C++ conversion layer attempts to address this issue by providing
  {\em automatic conversion} of indices. If a record field's name ends in
  \qq{\_index} (on the C++ side this corresponds to a field HIID ending in
  atomic ID \qq{"Index"}), and the field contains a single integer or a list of
  integers, then the field is assumed to contain indices, and conversion between
  0- and 1-base is automatically performed.}
  ...
  if forgotten about. This is why the paragraph above is higlighted in bold.
...
\section{MeqRequest and related data objects}
  ...
  \qq{cells}       & [optional] a \Cells\ object (see below)\\
  \qq{calc\_deriv} & [optional] compute perturbed values (0, 1 or 2).
                      Default is 0.\\
  \qq{next\_request} & [optional] a hint of what the next request is going to be.
                      This influences caching decisions and speculative
                      execution. See section~\ref{nextreq}. {\em (Not implemented yet)}.\\
  \qq{rider}       & [optional] rider subrecord containing additional commands.
  \\\hline
...
\subsection{Constructing Requests}
  ...
  and \qq{meq.add\_state()}. This is also described in section\ref{CSR}.
...
\subsection{Domains and Cells}
  ...
  functions defined over abstract 2D space (to be precice, with 
  ...
  contain the starting ($i',j'$) and ending ($i'',j''$) indices\footnote{1-based in
  Glish, 0-based in C++} of each regular segment. By definition,
  given $n$ segments and $N$ grid points, $i'_{k}= i''_k+1$, $i'_1=1$,
  $i''_n=N.$
  ...
  any of the values inside can invalidate the relationships between the grid,
  the domain, and the regular segments info, and lead to all sorts of confusion
  down the line.
...
\subsubsection{Constructing \Domain{}s and \Cells{}}
  ...
  letting the constructor figure out the domain \& cell size automatically. The
  two forms can even be mixed:
  
  \qq{~~cells := meq.cells(meq.domain(0,0,0,1),freq\_grid=[0.25,0.75],num\_time=2);}
  
  produces the same \Cells\ yet again -- note that \qq{freq\_grid} given here
  overrides the \qq{freq} component of the domain.
...
\section{MeqResult and related data objects}
  ...
  \noindent\begin{tabular}{l|p{.8\textwidth}}
  \hline
  \qq{cells}  & the \Cells\ of the result (not necessarily matching the request cells
  -- see discussion below)\\
  \qq{values} & the values of the result -- a list of \VellSet\ objects.\\
  \qq{integrated} & boolean flag indicating if the values are integrations or
  ...
  Note also that the \qq{values} field is defined as a {\em list} of
  \VellSet{}s. 
  ...
  fail-result record does not have a \qq{cells} field, and its \qq{values}
...
\subsection{VellSet}
  ...
  record has three forms:
  
  \paragraph{The normal form} contains the following fields:
  ...
  \qq{perturbations\_1}  & second set of parameter 
                        perturbations\\
  \qq{perturbed\_value\_1} & second set of perturbed values\\
  \hline
  \end{tabular}\vspace{1em}
  
  \paragraph{The empty form} of a \VellSet\ is just an empty record,
  corresponding to a default-constructed (empty) object in C++. While empty
  \VellSet{}s shouldn't be present in well-formed results, they can still
  appear in node state records and other structures, thus Glish code should be
  prepared to deal with them.

  \paragraph{The fail VellSet} indicates a run-time error or other failure.
  The corresponding record has only one field named \qq{fail}, containing a list
  of fail-records.
