...
def compute_piercings (ns,source_list):
  ...
  ns.pa_time0 = Meq.Compounder(
      ...
      ns.pa_time00 << Meq.ParAngle(xyz=Context.array.xyz0(),radec=radec0),
      common_axes=[hiid("time"),hiid("freq")]);
  if iono_rotate:
    ns.pa1 << ns.pa_time0 - (ns.pa << Meq.ParAngle(xyz=Context.array.xyz0(),radec=radec0));
    ns.P0 << Jones.define_rotation_matrix(ns.pa1);
    
  for p in Context.array.stations():
    xy = ns.xy(p) << Meq.Selector(xyz(p) - xyz(Context.array.stations()[0]),index=[0,1],multi=True);
    # if ionosphere is stuck to the skystcuk to sky, create P (p/a rotation matrix)
    if iono_rotate:
      ns.xy_r(p) << Meq.MatrixMultiply(ns.P0,xy);
    else:
      ns.pa1(p) << ns.pa_time0-(ns.pa(p)<<Meq.ParAngle(xyz=Context.array.xyz0(),radec=radec0));
