...
def mbar_model (ns,basename,l0,m0,dl,dm,nsrc):
  """Returns sources arranged in a line along the m axis""";
  model = [];
  model.append(point_source(ns,basename+"+0",l0,m0));
  for dy in range(-nsrc,nsrc+1):
    if dy:
      name = "%s%+d" % (basename,dy);
      model.append(point_source(ns,name,l0,m0+dm*dy));
  return model;

def lbar_model (ns,basename,l0,m0,dl,dm,nsrc):
  """Returns sources arranged in a line along the m axis""";
  model = [];
  model.append(point_source(ns,basename+"+0",l0,m0));
  for dx in range(-nsrc,nsrc+1):
    if dx:
      ...
      model.append(point_source(ns,name,l0+dl*dx,m0));
  return model;
